import { createI18n } from 'vue-i18n';

const messages = {
  en: {
    common: {
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      edit: 'Edit',
      add: 'Add',
      loading: 'Loading...',
      logout: 'Logout',
      backlog: 'Backlog',
      sprints: 'Sprints',
      columns: 'Columns',
      users: 'Users',
      dashboard: 'Dashboard',
      allSprints: 'All Sprints',
      active: 'Active',
      new_task: 'New Task',
      new_subtask: 'New Subtask',
      edit_task: 'Edit Task',
      edit_subtask: 'Edit Subtask',
      povlecite_sem: 'Drag here',
      title: 'Title',
      description: 'Description',
      priority: 'Priority',
      column: 'Column',
      sprint: 'Sprint',
      assignee: 'Assignee',
      author: 'Author',
      comments: 'Comments',
      no_sprint: 'No Sprint',
      low: 'Low',
      medium: 'Medium',
      high: 'High',
      login: 'Login',
      username: 'Username',
      password: 'Password',
      welcome: 'Welcome back to your project.',
      session_expired: 'Session expired. Please log in again.',
      none: 'None',
      subtasks: 'Subtasks',
      no_subtasks: 'No subtasks',
      updatedAt: 'Updated At',
      created: 'Created',
      attachments: 'Attachments',
      upload: 'Upload',
      download: 'Download',
      no_attachments: 'No attachments',
      expand: 'Expand',
      collapse: 'Collapse',
      history: 'Change History',
      boards: 'Boards',
      all_boards: 'All Boards',
      select_board: 'Select Board',
      manage_boards: 'Manage Boards',
      delete_confirm: 'Are you sure you want to delete this item?',
      due_date: 'Due Date',
      unknown: 'Unknown',
      completed: 'Completed',
      period: 'Period',
      status: 'Status',
      actions: 'Actions',
      write_comment: 'Write a comment...',
      no_comments: 'No comments yet. Be the first to comment!',
      change: 'Change',
      remove: 'Remove',
      success: {
        saved: 'Changes saved successfully.',
        created: 'Created successfully.',
        deleted: 'Deleted successfully.',
        uploaded: 'File uploaded successfully.',
        admin_seeded: "Admin (admin/Admin123!) created successfully."
      },
      error: {
        save: 'Error while saving.',
        delete: 'Error while deleting.',
        upload: 'Error while uploading file.',
        download: 'Error while downloading file.',
        seed_admin: 'Error while seeding admin.',
        file_too_large: 'File is too large (max 2MB).'
      },
      seed_admin: 'Seed Admin User (development)',
      rights: 'All rights reserved.',
      no_items: 'No items available',
      dark_mode: 'Dark Mode',
      light_mode: 'Light Mode',
      table_view: 'Table View',
      kanban_view: 'Kanban View',
      search_placeholder: 'Search tasks...',
      search: 'Search',
      no_results: 'No matching items found.',
      select_date: 'Select date',
      today: 'Today',
      close: 'Close',
      settings: 'Settings',
      appearance: 'Appearance',
      mailing: 'Mailing',
      notifications: 'Notifications',
      shortcuts: 'Shortcuts',
      notification_settings: 'Notification Settings',
      notification_desc: 'Configure how you want to receive notifications.',
      teams_webhook: 'Microsoft Teams Webhook URL',
      teams_info_title: 'How to get a Teams Webhook?',
      teams_info_text: 'In Microsoft Teams, go to your channel, click "..." -> "Connectors" -> "Incoming Webhook". Copy the generated URL here.',
      teams_test_button: 'Test Webhook',
      teams_test_success: 'Test notification sent to Teams successfully!',
      teams_test_error: 'Failed to send test notification to Teams. Check the URL.',
      unassigned_shortcut: 'Unassigned Tasks',
      settings_shortcut: 'Settings',
      all_tasks_shortcut: 'All Tasks',
      shortcut_help: 'Press "?" to see shortcuts',
      new_task_shortcut: 'New Task',
      search_shortcut: 'Search',
      help_shortcut: 'Show Shortcuts',
      esc_shortcut: 'Close / Cancel',
      kanban_shortcut: 'Kanban View',
      table_shortcut: 'Table View',
      my_tasks_shortcut: 'My Tasks Only',
      quick_filters: 'Quick Filters',
      filter_priority_high: 'High Priority',
      filter_due_soon: 'Due Soon',
      filter_my_tasks: 'My Tasks',
      clear_filters: 'Clear Filters',
      font_size: 'Font Size',
      font_family: 'Font Family',
      modal_size: 'Default Modal Size',
      modal_normal: 'Normal',
      modal_expanded: 'Expanded',
      very_small: 'Very Small',
      small: 'Small',
      medium_size: 'Medium',
      large: 'Large',
      very_large: 'Very Large',
      fonts: {
        sans: 'Sans Serif (Inter)',
        serif: 'Serif (Lora)',
        mono: 'Monospace (JetBrains Mono)',
        display: 'Display (Lexend)',
        comic: 'Comic (Comic Neue)',
        retro: 'Retro (Press Start 2P)'
      }
    },
    auth: {
      login_btn: 'Log In',
      error_login: 'An error occurred during login.',
      session_expired: 'Your session has expired. Please log in again.'
    },
    backlog: {
      delete_confirm: 'Are you sure you want to delete this task?',
      save_error: 'Error saving: '
    },
    sprints: {
      manage: 'Manage Sprints',
      add: 'Add Sprint',
      edit: 'Edit Sprint',
      name: 'Sprint Name',
      start: 'Start',
      end: 'End',
      active_sprint: 'Active Sprint',
      no_sprints: 'No sprints created.',
      delete_confirm: 'Are you sure you want to delete this sprint?',
      save_error: 'Error saving sprint.',
      delete_error: 'Error deleting sprint.'
    },
    columns: {
      manage: 'Manage Columns',
      add: 'Add Column',
      name: 'Column Name',
      color: 'Color',
      order: 'Order',
      delete_confirm: 'Are you sure you want to delete this column?',
      new_column: 'New Column',
    },
    users_mng: {
      manage: 'Manage Users',
      add: 'Add User',
      edit: 'Edit User',
      email: 'Email',
      role: 'Role',
      tags: 'Tags',
      tags_hint: 'Separate with comma (e.g., dev, frontend)',
      no_users: 'No users found.',
      delete_confirm: 'Are you sure you want to delete this user?',
      password_hint: 'Leave empty if you do not want to change',
      profile_picture_url: 'Profile Picture URL',
      profile_picture: 'Profile Picture',
    },
    notifications: {
      title: 'Notifications',
      mark_all_read: 'Mark all as read',
      no_notifications: 'No notifications',
      task_assigned: 'Task Assigned',
      task_removed: 'Removed from Task'
    },
    settings: {
      smtp: {
        title: 'SMTP Settings',
        description: 'Configure your SMTP server to enable email notifications.',
        host: 'SMTP Host',
        port: 'Port',
        userName: 'Username',
        password: 'Password',
        passwordPlaceholder: '••••••••',
        passwordHelp: 'Leave empty to keep existing password.',
        fromEmail: 'From Email',
        fromName: 'From Name',
        enableSsl: 'Enable SSL/TLS',
        saveSuccess: 'SMTP settings saved successfully.',
        saveError: 'Failed to save SMTP settings.',
        testTitle: 'Test Connection',
        testPlaceholder: "recipient{'@'}example.com",
        testButton: 'Send Test Email',
        testSuccess: 'Test email sent successfully!',
        testError: 'Failed to send test email. Check your settings.',
        mailtrapGuideTitle: 'Mailtrap Quick Config'
      }
    },
    dashboard: {
      personal: 'Personal',
      general: 'General',
      all: 'All Tasks',
      me: 'My Tasks',
      unassigned: 'Unassigned',
      notepad: 'Notepad'
    },
    notepad: {
      title: 'Notepad',
      new_note: 'New Note',
      search: 'Search notes...',
      empty: 'No notes yet.',
      no_results: 'No matching notes.',
      create_first: 'Create your first note',
      untitled: 'Untitled',
      no_content: 'No content',
      select_or_create: 'Select a note or create a new one',
      confirm_delete: 'Are you sure you want to delete this note?',
      last_saved: 'Last saved',
      autosave: 'Auto-save on',
      saving: 'Saving...',
      start_writing: 'Start writing...',
      bold: 'Bold',
      italic: 'Italic',
      underline: 'Underline',
      heading1: 'Heading 1',
      heading2: 'Heading 2',
      paragraph: 'Paragraph',
      bullet_list: 'Bullet List',
      ordered_list: 'Numbered List',
      delete: 'Delete note'
    }
  },
  sl: {
    common: {
      save: 'Shrani',
      cancel: 'Prekliči',
      delete: 'Izbriši',
      edit: 'Uredi',
      add: 'Dodaj',
      loading: 'Nalaganje...',
      logout: 'Odjava',
      backlog: 'Seznam opravil',
      sprints: 'Sprinti',
      columns: 'Stolpci',
      users: 'Uporabniki',
      dashboard: 'Pregledna plošča',
      allSprints: 'Vsi sprinti',
      active: 'Aktiven',
      new_task: 'Novo opravilo',
      new_subtask: 'Novo podopravilo',
      edit_task: 'Uredi opravilo',
      edit_subtask: 'Uredi podopravilo',
      povlecite_sem: 'Povlecite sem',
      title: 'Naslov',
      description: 'Opis',
      priority: 'Prioriteta',
      column: 'Stolpec',
      sprint: 'Sprint',
      assignee: 'Izvajalec',
      author: 'Avtor',
      comments: 'Komentarji',
      no_sprint: 'Brez sprinta',
      low: 'Nizka',
      medium: 'Srednja',
      high: 'Visoka',
      login: 'Prijava',
      username: 'Uporabniško ime',
      password: 'Geslo',
      welcome: 'Dobrodošli nazaj v vaš projekt.',
      session_expired: 'Seja je potekla. Prosimo, prijavite se ponovno.',
      none: 'Nihče',
      subtasks: 'Podopravila',
      no_subtasks: 'Ni podopravil',
      updatedAt: 'Posodobljeno',
      created: 'Ustvarjeno',
      attachments: 'Priponke',
      upload: 'Naloži',
      download: 'Prenesi',
      no_attachments: 'Ni priponk',
      expand: 'Razširi',
      collapse: 'Strni',
      history: 'Zgodovina sprememb',
      boards: 'Plošče',
      all_boards: 'Vse plošče',
      select_board: 'Izberi ploščo',
      manage_boards: 'Upravljanje plošč',
      delete_confirm: 'Ali ste prepričani, da želite izbrisati ta element?',
      due_date: 'Rok izdelave',
      unknown: 'Neznano',
      completed: 'Zaključen',
      period: 'Obdobje',
      status: 'Status',
      actions: 'Akcije',
      write_comment: 'Napiši komentar...',
      no_comments: 'Ni komentarjev. Bodite prvi, ki boste komentirali!',
      change: 'Spremeni',
      remove: 'Odstrani',
      success: {
        saved: 'Spremembe shranjene.',
        created: 'Ustvarjeno uspešno.',
        deleted: 'Izbrisano uspešno.',
        uploaded: 'Datoteka naložena.',
        admin_seeded: "Admin (admin/Admin123!) je bil uspešno ustvarjen."
      },
      error: {
        save: 'Napaka pri shranjevanju.',
        delete: 'Napaka pri brisanju.',
        upload: 'Napaka pri nalaganju datoteke.',
        download: 'Napaka pri prenosu datoteke.',
        seed_admin: 'Napaka pri sejanju admina.',
        file_too_large: 'Datoteka je prevelika (največ 2MB).'
      },
      seed_admin: 'Sejaj Admin uporabnika (razvoj)',
      no_items: 'Ni elementov',
      rights: 'Vse pravice pridržane.',
      dark_mode: 'Temni način',
      light_mode: 'Svetli način',
      table_view: 'Tabelarni prikaz',
      kanban_view: 'Kanban prikaz',
      search_placeholder: 'Išči naloge...',
      search: 'Išči',
      no_results: 'Ni zadetkov za iskanje.',
      select_date: 'Izberi datum',
      today: 'Danes',
      close: 'Zapri',
      settings: 'Nastavitve',
      appearance: 'Videz',
      mailing: 'E-pošta',
      notifications: 'Obvestila',
      shortcuts: 'Bližnjice',
      notification_settings: 'Nastavitve obvestil',
      notification_desc: 'Nastavite, kako želite prejemati obvestila.',
      teams_webhook: 'Microsoft Teams Webhook URL',
      teams_info_title: 'Kako pridobiti Teams Webhook?',
      teams_info_text: 'V Microsoft Teams pojdite v svoj kanal, kliknite "..." -> "Connectors" -> "Incoming Webhook". Kopirajte generiran URL sem.',
      teams_test_button: 'Preizkusi Webhook',
      teams_test_success: 'Testno obvestilo je bilo uspešno poslano v Teams!',
      teams_test_error: 'Pošiljanje testnega obvestila v Teams ni uspelo. Preverite URL.',
      unassigned_shortcut: 'Nedodeljena opravila',
      settings_shortcut: 'Nastavitve',
      all_tasks_shortcut: 'Vsa opravila',
      shortcut_help: 'Pritisnite "?" za bližnjice',
      new_task_shortcut: 'Nova naloga',
      search_shortcut: 'Išči',
      help_shortcut: 'Prikaži bližnjice',
      esc_shortcut: 'Zapri / Prekliči',
      kanban_shortcut: 'Kanban prikaz',
      table_shortcut: 'Tabelarni prikaz',
      my_tasks_shortcut: 'Samo moja opravila',
      quick_filters: 'Hitri filtri',
      filter_priority_high: 'Visoka prioriteta',
      filter_due_soon: 'Rok poteče kmalu',
      filter_my_tasks: 'Moje naloge',
      clear_filters: 'Počisti filtre',
      font_size: 'Velikost pisave',
      font_family: 'Vrsta pisave',
      modal_size: 'Privzeta velikost modalov',
      modal_normal: 'Običajna',
      modal_expanded: 'Razširjena',
      very_small: 'Zelo majhna',
      small: 'Majhna',
      medium_size: 'Srednja',
      large: 'Velika',
      very_large: 'Zelo velika',
      fonts: {
        sans: 'Brezserifna (Inter)',
        serif: 'Serifna (Lora)',
        mono: 'Široka (JetBrains Mono)',
        display: 'Naslovna (Lexend)',
        comic: 'Stripovska (Comic Neue)',
        retro: 'Retro (Press Start 2P)'
      }
    },
    auth: {
      login_btn: 'Prijavi se',
      error_login: 'Prišlo je do napake pri prijavi.',
      session_expired: 'Vaša seja je potekla. Prosimo, prijavite se znova.'
    },
    backlog: {
      delete_confirm: 'Ali ste prepričani, da želite izbrisati to opravilo?',
      save_error: 'Napaka pri shranjevanju: '
    },
    sprints: {
      manage: 'Upravljanje sprintov',
      add: 'Dodaj sprint',
      edit: 'Uredi sprint',
      name: 'Ime sprinta',
      start: 'Začetek',
      end: 'Konec',
      active_sprint: 'Aktiven sprint',
      no_sprints: 'Ni ustvarjenih sprintov.',
      delete_confirm: 'Ali ste prepričani, da želite izbrisati ta sprint?',
      save_error: 'Napaka pri shranjevanju sprinta.',
      delete_error: 'Napaka pri brisanju sprinta.'
    },
    columns: {
      manage: 'Upravljanje stolpcev',
      add: 'Dodaj stolpec',
      name: 'Ime stolpca',
      color: 'Barva',
      order: 'Vrstni red',
      delete_confirm: 'Ali ste prepričani, da želite izbrisati ta stolpec?',
      new_column: 'Nov stolpec',
    },
    users_mng: {
      manage: 'Upravljanje uporabnikov',
      add: 'Dodaj uporabnika',
      edit: 'Uredi uporabnika',
      email: 'Email',
      role: 'Vloga',
      tags: 'Oznake',
      tags_hint: 'Ločite s vejico (npr. dev, frontend)',
      no_users: 'Ni najdenih uporabnikov.',
      delete_confirm: 'Ali ste prepričani, da želite izbrisati tega uporabnika?',
      password_hint: 'Pustite prazno, če ne želite spremeniti',
      profile_picture_url: 'URL profilne slike',
      profile_picture: 'Profilna slika',
    },
    notifications: {
      title: 'Obvestila',
      mark_all_read: 'Označi vse kot prebrano',
      no_notifications: 'Ni obvestil',
      task_assigned: 'Dodeljeno opravilo',
      task_removed: 'Odstranjeni iz opravila'
    },
    settings: {
      smtp: {
        title: 'SMTP Nastavitve',
        description: 'Konfigurirajte SMTP strežnik za omogočanje e-poštnih obvestil.',
        host: 'SMTP Gostitelj',
        port: 'Vrata (Port)',
        userName: 'Uporabniško ime',
        password: 'Geslo',
        passwordPlaceholder: '••••••••',
        passwordHelp: 'Pustite prazno, če želite ohraniti obstoječe geslo.',
        fromEmail: 'E-pošta pošiljatelja',
        fromName: 'Ime pošiljatelja',
        enableSsl: 'Omogoči SSL/TLS',
        saveSuccess: 'SMTP nastavitve so bile uspešno shranjene.',
        saveError: 'Napaka pri shranjevanju SMTP nastavitev.',
        testTitle: 'Preizkus povezave',
        testPlaceholder: "prejemnik{'@'}primer.si",
        testButton: 'Pošlji testno e-pošto',
        testSuccess: 'Testna e-pošta je bila uspešno poslana!',
        testError: 'Pošiljanje testne e-pošte ni uspelo. Preverite nastavitve.',
        mailtrapGuideTitle: 'Mailtrap hitra nastavitev'
      }
    },
    dashboard: {
      personal: 'Osebno',
      general: 'Splošno',
      all: 'Vsa opravila',
      me: 'Moja opravila',
      unassigned: 'Nedodeljena',
      notepad: 'Beležnik'
    },
    notepad: {
      title: 'Beležnik',
      new_note: 'Nova beležka',
      search: 'Išči beležke...',
      empty: 'Še ni beležk.',
      no_results: 'Ni zadetkov.',
      create_first: 'Ustvari prvo beležko',
      untitled: 'Brez naslova',
      no_content: 'Brez vsebine',
      select_or_create: 'Izberi beležko ali ustvari novo',
      confirm_delete: 'Ali ste prepričani, da želite izbrisati to beležko?',
      last_saved: 'Zadnjič shranjeno',
      autosave: 'Samodejno shranjevanje',
      saving: 'Shranjevanje...',
      start_writing: 'Začnite pisati...',
      bold: 'Krepko',
      italic: 'Poševno',
      underline: 'Podčrtano',
      heading1: 'Naslov 1',
      heading2: 'Naslov 2',
      paragraph: 'Odstavek',
      bullet_list: 'Seznam z alinejami',
      ordered_list: 'Oštevilčen seznam',
      delete: 'Izbriši beležko'
    }
  }
};

const i18n = createI18n({
  legacy: false,
  locale: 'en',
  fallbackLocale: 'en',
  messages,
});

export default i18n;
