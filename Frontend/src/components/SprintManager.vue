  ...
  <div>
    ...
    <form @submit.prevent="handleSubmit" v-if="showForm" class="mb-8 p-6 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-3xl border border-indigo-100 dark:border-indigo-900/40 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300 shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label class="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1.5 ml-1 tracking-widest">{{ $t('sprints.start') }}</label>
          <input v-model="startDate" type="date" required class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-white dark:bg-slate-800 dark:text-slate-100 transition-all">
        </div>
        <div>
          <label class="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1.5 ml-1 tracking-widest">{{ $t('sprints.end') }}</label>
          <input v-model="endDate" type="date" required class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-white dark:bg-slate-800 dark:text-slate-100 transition-all">
        </div>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-indigo-100/50 dark:border-indigo-900/30">
    ...
    <div class="flex justify-between items-center mb-4">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-black text-slate-400 uppercase tracking-widest">{{ $t('common.sprints') }} ({{ sprints.length }})</span>
      </div>
      <button 
        v-if="!showForm"
        @click="showForm = true" 
      ...
      <table class="w-full text-left">
        <thead>
          <tr class="text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
            <th class="px-4 py-3">{{ $t('sprints.name') }}</th>
        ...
        <tbody class="divide-y divide-slate-50 dark:divide-slate-900/40">
          <tr v-for="s in sprints" :key="s.id" class="group hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition-all">
            <td class="px-4 py-4">
            ...
            <td class="px-4 py-4 text-right">
              <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                <button @click="editSprint(s)" class="p-2 text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-all shadow-sm" :title="$t('common.edit')">
                  <Edit2 :size="16" />
                </button>
                <button @click="deleteSprint(s.id)" class="p-2 text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl transition-all shadow-sm" :title="$t('common.delete')">
                  <Trash2 :size="16" />
                </button>
              </div>
            </td>
